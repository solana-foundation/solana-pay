...
Supply `PAY_PAYMENT_RECIPIENT`, `PAY_RPC_URL`, `PAY_BENCH_OPERATOR_KEYPAIR`,
and a persistent random `PAY_SESSION_SECRET` through the service environment.
The operator keypair may be a Solana JSON byte array or base58-encoded 64-byte
keypair. The gateway advertises `feePayer: true`, co-signs each validated open,
and pays the transaction fee plus channel-account rent. A real devnet load run
still requires the bench funder to hold devnet SOL and USDC; `pay-bench run`
currently stops before funding rather than pretending it completed that
transfer.
